...
ただ、ここは**うさぎの真似ではありません**。うさぎは1人ですから、10体で押し寄せてはくれない。**同じ振る舞いのものを何体でも並べられる**のは、システム側だけに許された特権です。1体でやり切ってしまううさぎの方がすごい、という話でもあります。
...
ここで1つ、**API を有効にしただけでは足りないもの**があります。**Firestore** です。

```bash
# 冪等性の記録を置くデータベースを作る（(default) という名前で1つできる）
gcloud firestore databases create --location="${REGION}"
```

`firestore.googleapis.com` を有効にするのは「**この機能を使う権利を開ける**」だけで、**データベースの実体は別途作る必要があります**。作らずに動かすと、うさぎが湧いた瞬間にこう言われます。

```
404 The database (default) does not exist for project ...
```

さっきのサービスエージェントと同じ構図ですね。**権利と実体は別。** クラウドを触っていると、この2つがずれている状態にちょくちょく出くわします。

なお、**ここで指定したロケーションは後から変えられません**。データベースを作り直すしかなくなるので、Cloud Run と同じ場所（今回は東京）にしておきます。
...
タグは `1.0` でも `v2` でも、自分がわかる名前で構いません。ただ**`latest` だけはやめておいた方がいいです**。

`latest` は便利ですが、**同じ名前のまま中身が入れ替わります**。障害が起きたときに「そのとき動いていた `latest`」を後から取り出せないし、「今動いているうさぎが、どのソースから焼かれたのか」も追えません。せっかく `uv.lock` で依存を固定したのに、イメージの名前の側が動くと、再現性がそこで切れてしまいます。

**焼くたびに違う名前をつける。** それだけで、後から「あのときのうさぎ」を呼び戻せるようになります。運用に乗せるなら、この名札をリリース番号やコミットハッシュにしておくと、イメージとソースが1対1で対応してさらに追いやすくなります。
...
- 大量に叩かれて、**うさぎ役のコンテナが100体湧いて課金が伸びる**（ゼロスケールは、ゼロから青天井への近道でもあります）
...
Service account service-＜プロジェクト番号＞@gs-project-accounts.iam.gserviceaccount.com does not exist.
```
...
代理人が先に必要なら、こちらから一度呼んで、存在してもらってから権限を渡す。順番が1つ増えるだけの話ですが、知らないと「名前は合っているのに存在しないと言われる」で30分溶かします（溶かしました）。

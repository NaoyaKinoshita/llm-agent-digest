...
**白石**: このデータセンター、もともとMicrosoftの枠だったやつじゃないですか。
...
**相馬**: 手元で動くAIエージェントの話もありました。これがまた嫌な話で。
...
**相馬**: しかも、書いてあるとおりに動いてしまう。

**相馬**: 先々週号の、Hugging Face侵入で学習が止まっていた話。続報が出ましたね。
...
**白石**: METRとRedwood Researchは無報酬で6日間調査したそうです。無報酬で、6日。

**黒木**: その6日、たぶん誰かの週末が消えています。

**白石**: ……それでも第三者検証が入っているのは、素直にありがたいです。

**諏訪**: 検証があること自体は良いことです。ただ、報告書が出たのは事件から1か月以上あとです。原因の特定と対策の実装の間には、常にこれだけの時間差があります。

**相馬**: 週の最後には、100社超の共同書簡も出ました。
...
**白石**: 2万8,000人ですよ。全員の隣にエージェントが座るわけです。壮観じゃないですか。
...
**相馬**: SalesforceとAnthropicの提携も、その仲間ですか。
...
**白石**: CRMを開かずに営業データが触れるんですよ。画面の行き来がなくなるだけで、もう嬉しい。
